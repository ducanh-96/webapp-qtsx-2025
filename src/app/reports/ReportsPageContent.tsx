...
export function ReportsPageContent() {
  const [reports, setReports] = React.useState<PowerBIReportInfo[]>([]);
  const [selectedReportId, setSelectedReportId] = React.useState<string>('');
  const [error, setError] = React.useState<string | null>(null);
  const [dynamicHeight, setDynamicHeight] = React.useState<string>('80vh');
  const [isFullscreen, setIsFullscreen] = React.useState(false);
  const [prevHeight, setPrevHeight] = React.useState<string>('80vh');
